...
/**
 * This is a frame class which could generate an assessment frame for SHA algorithm.
 * @author aaafc
 *
 */
public class MySHAAssessDisplay extends JFrame implements MyProcessAssessDisplay{
	
	private static MySHAAssessDisplay theInstance = null;
	public static MySHAAssessDisplay getInstance()
	{
		if(theInstance == null)
			theInstance = new MySHAAssessDisplay();
		return theInstance;
	}
...
//	private JScrollBar scrollBar;
	...
	private MySHAAssessment assessment;
	
	private MySHAAssessDisplay() {
		assessment = new MySHAAssessment();
...
//		getContentPane().add(labelBits);
		...
		numberOfRoundsField.setBounds(163, 200, 66, 40);
		getContentPane().add(numberOfRoundsField);
		
		doProcess = new JButton("doHashing");
		...
		doProcess.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e)
			{
				try {
					...
					long temp = MySHAAssessDisplay.this.assessment.
							averageHashingProcess(messageL, rounds);
				...
				}catch(NumberFormatException numForExp)
				{
					...
					JOptionPane.showMessageDialog(MySHAAssessDisplay.this, "Invalid value format.");
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
					JOptionPane.showMessageDialog(MySHAAssessDisplay.this, "Exception: " 
		...
		tableColumnVector.add("AvgTimeOfHashing");
